...
def update_config(config: dict, config_combination: Combination) -> dict:
    ...
    config["trainer"]["LoRA"]["adapter_name"] = (
        f"SAM2_LoRA_{config_combination.modelname}"
    )
...
def write_config(config_combination: Combination, root_dir: Path) -> None:
    save_path = (
        root_dir
        / f"{config_combination.datasetname}_LoRA{config_combination.rank}_{config_combination.modelname}.yaml"
    )
...
def create_combinations() -> List[Combination]:
    combinations = list(
        product(LORA_RANKS, CHECKPOINT_PATHS.keys(), DATASETS_PATHS.keys())
    )
    combinations = [
        Combination(x[0], CHECKPOINT_PATHS[x[1]], DATASETS_PATHS[x[2]])
