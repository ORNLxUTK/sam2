...
def update_config(config: dict, config_combination: Combination) -> dict:
    ...
    config["submitit"]["name"] = f"FT_{config_combination.datasetname[0]}_{config_combination.modelname[0]}"
    return config

def write_config(config_combination: Combination, root_dir: Path) -> None:
    assert root_dir.exists() and root_dir.is_dir(), f"Root directory {root_dir} does not exist or is not a directory."
    save_path = root_dir / f"{config_combination.datasetname}_FullFinetune_{config_combination.modelname}.yaml"
...
def create_combinations() -> List[Combination]:
    ...
    combinations = [Combination(CHECKPOINT_PATHS[x[0]], DATASETS_PATHS[x[1]]) for x in combinations]
    return combinations
...
if __name__ == "__main__":
    ...
    for config in config_combinations:
        write_config(config, Path("/lustre/isaac24/proj/UTK0388/DomainSpecific/sam2/sam2/configs/sam2.1_training/FullFinetuneablations"))
    """
    submit_jobs(Path("/lustre/isaac24/proj/UTK0388/DomainSpecific/sam2/sam2/configs/sam2.1_training/FullFinetuneablations"))