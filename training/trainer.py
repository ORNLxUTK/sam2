...
class Trainer:
    ...
    def val_epoch(self, val_loader, phase):
        ...
        for data_iter, batch in enumerate(val_loader):
            ...
            torch.cuda.empty_cache()
            self.mem_before_batch = torch.cuda.memory_allocated()
            batch = batch.to(self.device, non_blocking=True)
            self.mem_after_batch = torch.cuda.memory_allocated()
            logging.info(f"{'Val Batch':=^60}")
            logging.info(f"Phase {phase} Before Batch Memory Usage: {mb(self.mem_before_batch):.2f} MB")
            logging.info(f"Phase {phase} After Batch Memory Usage: {mb(self.mem_after_batch):.2f} MB")
            logging.info(f"Phase {phase} Batch Memory Increase: {mb(self.mem_after_batch - self.mem_before_batch):.2f} MB")
            logging.info(f"{'':=^60}")
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                ...
                logging.info(f"Phase {phase} Before Optimizer Step Memory Usage: {mb(self.mem_before_optimizer_step):.2f} MB")
                logging.info(f"Phase {phase} After Optimizer Step Memory Usage: {mb(self.mem_after_optimizer_step):.2f} MB")
                logging.info(f"Phase {phase} Optimizer Step Memory Increase: {mb(self.mem_after_optimizer_step - self.mem_before_optimizer_step):.2f} MB")
                logging.info(f"{'':=^60}")
    ...
    def _run_step(
        ...
        logging.info(f"{'Gradient Calculation':=^60}")
        logging.info(f"Phase {phase} Before Gradient Memory Usage: {mb(self.mem_before_backward):.2f} MB")
        logging.info(f"Phase {phase} After Gradient Memory Usage: {mb(self.mem_after_backward):.2f} MB")
        logging.info(f"Phase {phase} Gradient Memory Increase: {mb(self.mem_after_backward - self.mem_before_backward):.2f} MB")
